...
class Integration(Node):
    def __init__(self):
        ...
        self.wheel_l_sub = self.create_subscription(Float32, 'left_wheel_speed/raw', lambda msg: self.handle_wheel_desired(msg, 'left'))        #2
        self.wheel_r_sub = self.create_subscription(Float32, 'right_wheel_speed/raw', lambda msg: self.handle_wheel_desired(msg, 'right'))      #3

        #Publishers (CAN -> Topic)
        self.wheel_l_pub = self.create_publisher(Float32, 'left_wheel_speed/encoder', 10)        #4
        self.wheel_r_pub = self.create_publisher(Float32, 'right_wheel_speed/encoder', 10)       #5
    ...
    def timer_callback(self):
        while True:
            ...
            match can_msg.arbitration_id:
                case 4:
                    #handle right wheel
                    handle_wheel_encoder(can_msg.data, 'right')
                case 5:
                    #handle left wheel
                    handle_wheel_encoder(can_msg.data, 'left')
                case 6:
                    #handle auto mode
                    handle_auton_mode(can_msg.data)
                case 7:
                    #handle estop
                    handle_estop(can_msg.data)
                case _:
